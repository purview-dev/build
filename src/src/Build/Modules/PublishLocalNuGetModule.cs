...
public sealed class PublishLocalNuGetModule(
...
) : Module<CommandResult>
{
	...
	)
	{
		...
		context.Logger.LogInformation(
			"Publishing local NuGet packages to {LocalFeedPath}.",
			fullLocalFeedPath
		);
		...
		foreach (var package in packages)
		{
			...
			if (!settings.OverwriteExistingPackages && File.Exists(destinationPath))
			{
				context.Logger.LogInformation(
					"Package {Package} already exists in local feed. Skipping.",
					fileName
				);
				File.Delete(package);
...
#pragma warning restore CA1308 // Normalize strings to uppercase
				...
				foreach (var httpCachePath in httpCachePaths)
				{
					...
					[
						...
						Path.Combine(
							httpCachePath,
							"nupkg_" + loweredPackageId + "." + loweredVersion + ".dat"
						),
					];
		...
		if (settings.ShutdownDotnetBuilderServer)
		{
	...
	static async Task<PackageDetails> ParsePackageDetailsAsync(
		string artifact,
		CancellationToken cancellationToken
	)
	{
		using NuGet.Packaging.PackageArchiveReader packageReader = new(artifact);
