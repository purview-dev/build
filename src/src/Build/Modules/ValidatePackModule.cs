...
) : Module<PackValidationResult[]>
{
	...
	)
	{
		...
		{
			var result = await ValidateNupkgAsync(
				package,
				packValidationSettings.Value,
				cancellationToken
			);
			results.Add(result);
		...
		{
			var result = await ValidateSnupkgAsync(
				package,
				packValidationSettings.Value,
				cancellationToken
			);
	...
	)
	{
		...
		try
		{
			using PackageArchiveReader reader = new(packagePath);
			...
			if (PackageInspector.RequiresAssemblyInspection(settings))
			{
				...
				if (File.Exists(snupkgPath))
				{
					...
					await PackageInspector.ValidateAssembliesAsync(reader, snupkgReader, settings, errors, cancellationToken);
				}
				else
				{
					await PackageInspector.ValidateAssembliesAsync(reader, null, settings, errors, cancellationToken);
				}
			}

			PackValidationResult result = new(
	...
	)
	{
		...
		try
		{
			...
			if (nonSymbolFiles.Length > 0)
				errors.Add(
					$"Symbol package contains non-symbol file(s): {string.Join(", ", nonSymbolFiles)}."
				);
	...
	)
{
	...
	public string PackageKey { get; } = packageKey;
...
public sealed class PackagePair
{
